...
<template>
	<div class="relative mb-xl" v-if="steps.length">
		...
		<div
			class="absolute top-1/2 left-0 h-[10px] bg-primary rounded-full transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] -translate-y-1/2"
			:style="{ width: `${progress}%` }" />

		<div class="relative flex justify-between">
			<div v-for="(step, i) in steps" :key="step.id" class="flex flex-col items-center text-center w-full mb-lg">
				<div
					class="w-10 h-10 flex items-center justify-center rounded-full border transition-all duration-500"
					:class="[
						i + 1 <= activeStepIndex
							? 'bg-primary border-primary text-white'
							: 'border-black/80 text-black/80',
					]">
					<span class="text-md font-semibold">{{ i + 1 }}</span>
				</div>

				<p
					class="text-md leading-tight max-w-[80px] mt-lg"
