...
export default factories.createCoreController("api::subservice.subservice", ({ strapi }) => ({
	async find(ctx) {
		...
		const { results, pagination } = await strapi
			.service("api::subservice.subservice")
			.getAllSubservices(sanitizedQuery);
	...
	async findOne(ctx) {
		...
		const entity = await strapi.service("api::subservice.subservice").getSubserviceBySlug(sanitizedQuery, id);
